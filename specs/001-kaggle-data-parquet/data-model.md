...
本文档定义了 Kaggle 数据下载与 Parquet 格式转换功能所涉及的关键数据实体、属性和关系。这些模型将用于实现系统中的数据结构和操作流程，支持最多 10GB 的中型数据集，使用系统密钥环或环境变量安全存储 API 凭证，通过日志文件和状态文件跟踪处理过程，并在遇到 API 限流时采用指数退避重试策略。
...
Kaggle API 凭证将通过以下方式安全存储，按优先级顺序：

1. **系统密钥环**：使用操作系统提供的安全存储机制
   - Windows：Windows 凭证管理器
   - macOS：钥匙串访问
   - Linux：Secret Service API / GNOME Keyring

2. **环境变量**：作为备选方案，支持以下环境变量
   - `KAGGLE_USERNAME`：Kaggle 用户名
   - `KAGGLE_KEY`：Kaggle API 密钥

凭证决不会以明文形式存储在配置文件或日志中。

## 错误处理与恢复

系统将实现以下错误处理与恢复机制：

1. **断点续传**：记录下载进度，支持从中断处恢复
2. **完整性验证**：使用校验和验证下载文件的完整性
3. **重试机制**：对网络错误和 API 限流采用指数退避重试策略
4. **详细日志**：记录所有操作和错误，便于诊断和恢复
5. **预检验证**：在开始处理前验证磁盘空间、凭证有效性等条件