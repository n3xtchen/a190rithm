...
def test_tokenizer():
    ...
    train_data = train_data.map(lambda row: {
        "token_size": [
            len(i) for i in
            tokenizer(row['sentence'], add_special_tokens=True)["input_ids"]
        ]
    }, batched=True)
...
def test_text_classfication():
    ...
    def proc(text):
        """
        """
        token_plus = tokenizer.evaluate(text)
        return {
...
def test_text_classfication_inference():
    ...
    data : Dataset = load_dataset("glue", "cola")
    test_data: Dataset = data["validation"].map(lambda row: proc(row["sentence"]), batched=True)
