...
def test_text_classfication():
    ...
    data["train"] = data["train"].map(
        lambda row: {"token_size": len(tokenizer.tokenize(row['sentence']))})
    ...
    def proc(text: str):
        ...
        return {
            "input_ids": token_plus["input_ids"][0],
            "attention_mask": token_plus["attention_mask"][0]
        }
    train_data: Dataset = data["train"].map(lambda row: proc(row["sentence"]))
    train_data.set_format(type="torch", columns=["input_ids", "attention_mask", "label"])
    train_dataloader = DataLoader(train_data, batch_size=32)
    category_size = 2 # len(set(train_data["label"]))
    assert category_size == 2

    validate_data: Dataset = data["validation"].map(lambda row: proc(row["sentence"]))
    ...
    test_data: Dataset = data["test"].map(lambda row: proc(row["sentence"]))
